...
graph TD
    ...
    UserOperador["Operador / Técnico\n(Dispatcher)"]
    UserConductor["Conductor\n(Móvil - Modo Conductor)"]
    UserAuditor["Auditor Externo\n(Solo Lectura)"]
    UserCliente["Cliente Bodega/Mercado\n(WhatsApp/SMS)"]

    System["Sistema EcoLogística Lima\nPlataforma Web VRPTW + Green VRP\nSoftware System"]

    ExtAuth["Servicio Externo OAuth2\n(Opcional)"]
    ExtMail["Servicio SMTP Cloud\n(SendGrid / SES)"]
    ExtWaze["Waze CCP / SIMAT\nTráfico tiempo real"]
    ExtMaps["Google Maps API\n(Fallback) / OSM Tiles"]
    ExtPNP["PNP - Zonas de Riesgo\nAPI Datos Abiertos"]
    ExtIGN["IGN Perú / OpenStreetMap\nDatos Geográficos"]

    System -->|"Autentica (si SSO)"| ExtAuth
    System -->|"Envía notificaciones ETA/alerta"| ExtMail
    System -->|"Consulta congestión y bloqueos\nREST JSON"| ExtWaze
    System -->|"Geocodifica y renderiza tiles\nLeaflet"| ExtMaps
